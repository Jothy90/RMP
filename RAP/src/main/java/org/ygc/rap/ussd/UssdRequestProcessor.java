...
/**
    ...
    public void processRequest(final MoUssdReq moUssdReq) throws SdpException {
        ...
        if (USSD_OP_MO_INIT.equals(moUssdReq.getUssdOperation())) {
            ...
            device = DeviceDataLayer.getDeviceByMask(moUssdReq.getSourceAddress());
            if (device.getId() > 0) {
                serviceCode = "2";
            }
            deviceAccess = DeviceAccessDataLayer.getDeviceAccessByMask(moUssdReq.getSourceAddress());
            if (deviceAccess.getId()>0) {
                device= DeviceDataLayer.getDeviceById(deviceAccess.getDeviceId());
                serviceCode = "3";
            }
        } else {
